...
wait_ke_A:                              ; Wait Crypto_A.keyExchange = PORT_A.dataReg(10) = 1
    ...
    LDH R2, #MAGICNUMBER_A              ; STORE THE MAGICNUMBER FROM CRYPTOMESSAGE_A
    LDL R2, #MAGICNUMBER_A              ; R2 <- &MAGICNUMBER_A
    ST  R15, R2, R0                     ; MAGICNUMBER_A <- R15                                               
    ...
    LDH R2, #CRYPTOKEY_A                 ; STORE THE CRYPTOKEY_A
    LDL R2, #CRYPTOKEY_A                 ; R2 <- &CRYPTOKEY_A
    ST  R12, R2, R0                     ; MAGICNUMBER_A <- R12


while_msg_a:                            ; LOOP UNTIL EOM = 1
while_data_av_a:                        ; Wait Crypto_A.data_av = PORT_A.dataReg(9) = 1
    ...
    JMPZD #while_data_av_a              ; if PORT_A.dataReg(9) = 0 jump
    ...
    JMPZD #index_even_a                 ; IF R7 == 0, index is an even number
    ...
    LD  R11, R4, R9                     ; R11 <- MSG_C1[ INDEX_A ]
...
store_char_a:
    ...
    ST  R10, R4, R6                     ; MSG_C1[ INDEX_A ] <- R10
...
wait_ke_B:                              ; Wait Crypto_B.keyExchange = PORT_B.dataReg(10) = 1
    ...
    ; R15 = MAGICNUMBER OF CRYPTOMESSAGE_B

    ; Store cryptoMessage magicNumber
    LDH R2, #MAGICNUMBER_B              ; STORE THE MAGICNUMBER FROM CRYPTOMESSAGE_B
    LDL R2, #MAGICNUMBER_B              ; R2 <- &MAGICNUMBER_B
    ST  R15, R2, R0                     ; MAGICNUMBER_B <- R15                                               
    ...
    LDL  R15, MAGICNUMBER_B            ; R15 <- MAGICNUMBER_B FROM CRYPTOMESSAGE_B
    LDH  R14, RANDNUMBER
    LDL  R14, RANDNUMBER               ; R14(exponent) <- RANDNUMBER (variable, "random")
    LDH  R13, Q
    LDL  R13, Q                        ; R13 <- Q (prime number, constant)
    ...
    ; Store cryptographic key
    LDH R2, #CRYPTOKEY_B                 ; STORE THE CRYPTOKEY_B
    LDL R2, #CRYPTOKEY_B                 ; R2 <- &CRYPTOKEY_B
    ST  R12, R2, R0                     ; MAGICNUMBER_B <- R12

while_msg_b:                            ; LOOP UNTIL EOM = 1
while_data_av_b:                        ; Wait Crypto_B.data_av = PORT_B.dataReg(9) = 1
    ; Check if data_av flag is activated
    LDH  R1, PORT_B_ADDRESS             
    LDL  R1, PORT_B_ADDRESS             ; R1 <- &PORT_B                                        
    JSRD #crypto_get_data_av            ; CHECK DATA_AV FLAG
    ...
    JMPZD #while_data_av_b              ; if PORT_B.dataReg(9) = 0 jump
                                        ; ELSE THERE IS A MSG ON DATA_IN
    ...
    ; R15 contains the encrypted character form cryptoMessage_B device
                                                                                                     ; precisa fazer a logica de eom
    ; Send an ACK signal to cryptoMessage device
    JSRD #crypto_send_ack               ; SET ACK FOR SOME CYCLES
                                        ; param  : R1    <- &PORT.EnableReg 
    ...
    ; Load cryptographic key
    LDH  R3, #CRYPTOKEY_B
    LDL  R3, #CRYPTOKEY_B               ; R3 <- CRYPTOKEY_B
    LD   R3 , R3, R0
    ...
    ; Store decrypted character in the array and increment the index
    LDH R4, #MSG_C2
    LDL R4, #MSG_C2                     ; R4 <- &MSG_C2
    LDH R8, #INDEX_B
    LDL R8, #INDEX_B                    ; R8 <- &INDEX_B
    LD  R6, R8, R0                      ; R6 <- INDEX_B
    ...
    LDL R5, #01h
    AND R7, R6, R5                      ; R7 <- R6 and 1
    JMPZD #index_even_b                 ; IF R7 == 0, index is an even number
                                        ; else index is an odd number
...
store_char_b:
    ...
    ; Look if there was a EOM before
    LDH R2, #OEM_B
    LDL R2, #OEM_B                      ; R2 <- &EOM variable
    LD  R3, R2, R0                      ; R3 <- EOM variable
    ADD R3, R3, R0

    JMPZD #check_eom_b                  ; if zero, EOM flag didnt occur yet
                                        ; ELSE END OF MESSAGE Flag have occured
    
    ; When the code reaches this point, all characters of the message were read.
    ; Now we need to reset the INDEX_B value and change to cryptoMessage_B device
    LDH  R4, #INDEX_B
    LDL  R4, #INDEX_B
    ST   R0, R4, R0                     ; INDEX_B  <- 0
    ...
    ; Return to communication with cryptoMessage device A
    JMPD #wait_ke_A                    
...
check_eom_b:
    ...
    JMPZD #while_msg_b                  ; if PORT_A.dataReg(9) = 0 JUMP             Get other character from cryptMessage
                                        ; ELSE END OF MESSAGE
    ...
    LDH R3, #00h
    LDL R3, #01h                        ; R3 <- 1
    ST  R3, R2, R0                      ; OEM_B <- 1        This means that it is the last character to read

     JMPD #while_msg_b                   ; Return to wait new character
